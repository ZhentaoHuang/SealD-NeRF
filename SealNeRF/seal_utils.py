...
class SealBBoxMapper(SealMapper):
    ...
    def __init__(self, config_path: str, seal_config: object) -> None:
        ...
        # fill_meshes = Meshes([torch.from_numpy(mesh.vertices) for mesh in bound_mesh_list], [
        #     torch.from_numpy(mesh.faces) for mesh in bound_mesh_list])
        vertices_tensors = [torch.from_numpy(vert.setflags(write=True) or vert) for mesh in bound_mesh_list for vert in [mesh.vertices]]
        faces_tensors = [torch.from_numpy(face.setflags(write=True) or face) for mesh in bound_mesh_list for face in [mesh.faces]]
        fill_meshes = Meshes(vertices_tensors, faces_tensors)
